...
class BGEEmbedding:
    """Embedding provider using BGE-M3"""
...
class EntityLinker:
    ...
    def __init__(self):
        ...
        # 初始化BGE-M3嵌入模型
        self.embedding_provider = BGE_M3_Embedder()
    ...
    def normalize_entity(self, entity: str) -> str:
        """标准化实体名称，支持BGE-M3混合检索和领域词典增强"""
        if not entity:
            return entity
        entity_lower = entity.lower().strip()
        # 直接查找标准形式
        if entity_lower in self.entity_to_standard:
            return self.entity_to_standard[entity_lower]
        # 用领域词典所有词做混合检索
        dict_terms = list(self.standard_entities.keys())
        if not dict_terms:
            return entity
        query_emb = self.embedding_provider.embed([entity])
        dict_emb = self.embedding_provider.embed(dict_terms)
        sim = hybrid_similarity(query_emb, dict_emb, alpha=self.hybrid_alpha)[0]
        max_idx = int(np.argmax(sim))
        max_sim = float(sim[max_idx])
        if max_sim > self.sim_threshold:
            best_match = dict_terms[max_idx]
            self.logger.debug(f"BGE-M3混合检索: {entity} -> {best_match} (sim={max_sim:.3f})")
            return best_match
        return entity

    def enhance_domain_dict(self, new_terms: List[str], sim_threshold: float = None):
        """用BGE-M3自动扩充领域词典"""
        import json
        sim_threshold = sim_threshold or self.sim_threshold
        dict_terms = list(self.standard_entities.keys())
        term_to_cat = {t: self.standard_entities[t] for t in dict_terms}
        if not dict_terms:
            dict_emb = self.embedding_provider.embed(new_terms)
            dict_terms = new_terms
        else:
            dict_emb = self.embedding_provider.embed(dict_terms)
        new_emb = self.embedding_provider.embed(new_terms)
        ...
        for i, term in enumerate(new_terms):
            ...
            if max_sim > sim_threshold:
                ...
                if term not in self.domain_dict[cat]:
                    self.domain_dict[cat].append(term)
            else:
                self.domain_dict.setdefault("其他", []).append(term)
        # 去重
        for k in self.domain_dict:
            self.domain_dict[k] = list(sorted(set(self.domain_dict[k])))
        with open(self.domain_dict_path, "w", encoding="utf-8") as f:
            json.dump(self.domain_dict, f, ensure_ascii=False, indent=2)
        self.logger.info(f"领域词典已扩充，现有类别: {list(self.domain_dict.keys())}")
    ...
    def normalize_entity(self, entity: str) -> str:
        ...
        # 使用BGE-M3模型计算相似度
        embeddings = self.embedding_provider.embed([entity])
        best_match = None
        max_similarity = 0.8  # 设置相似度阈值
        
        for standard, variants in self.standard_entities.items():
            all_forms = [standard] + variants
            for form in all_forms:
                form_embeddings = self.embedding_provider.embed([form])
                similarity = np.dot(embeddings, form_embeddings.T).flatten()[0]
        ...
        if best_match:
            logger.debug(f"语义匹配: {entity} -> {best_match}")
            return best_match
    ...
    def _find_nearest_entity(self, text: str, pronoun: str, entities: List[str]) -> Optional[str]:
        """在文本中查找代词最近的实体
    ...
    def disambiguate_entities(self, entities_df: pd.DataFrame) -> pd.DataFrame:
        """实体消歧：处理多义实体
